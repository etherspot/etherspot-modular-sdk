...
var SessionKeyValidator_exports = {};
__export(SessionKeyValidator_exports, {
  SessionKeyValidator: () => SessionKeyValidator
});
module.exports = __toCommonJS(SessionKeyValidator_exports);
...
var SessionKeyValidator = class _SessionKeyValidator {
  ...
  static async create(modularSdk) {
    const sessionKeyValidator = new _SessionKeyValidator(modularSdk);
  ...
  async initialize(modularSdk) {
    ...
    const installed = await modularSdk.isModuleInstalled("0x01" /* VALIDATOR */, erc20SessionKeyValidator);
    if (!installed) {
  ...
  async signUserOpWithSessionKey(sessionKey, userOp) {
    try {
      ...
      const apiKeyMatch = this.providerURL.match(/api-key=([^&]+)/);
      const apiKey = apiKeyMatch ? apiKeyMatch[1] : null;
      const data = await this.getSignUserOp(
        account,
        chainId,
        apiKey,
        sessionKey,
...
*/
//# sourceMappingURL=SessionKeyValidator.cjs.map