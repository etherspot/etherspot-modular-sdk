...
// tsx examples/modules/credible-account-modules/uninstall-hook-multiplexer.ts
async function main() {
  ...
  const deinitData = encodeAbiParameters([{ type: 'uint256' }], 
                            [BigInt(MODULE_TYPE.HOOK)]);

  console.log(`deinitData: ${deinitData}`);

  const uoHash = await modularSdk.uninstallModule(MODULE_TYPE.HOOK, 
    networkConfig.contracts.hookMultiPlexer, deinitData);
